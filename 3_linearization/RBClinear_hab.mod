...
parameters 
    ...
    chii            ${\chi}$                     (long_name='Habitat parameter')
;
...
%----------------------------------------------------------------
    ...
    H_bar=alp*(1-varrho)/(cy*varrho*(1-chii)+alp*(1-varrho));// Steady state hours
...
[name='Marginal Utility of Consumption']
    ...
    uc=((1-varrho)*(1-sigma_c)-1)*((c-chii*c(-1))/(1-chii))+ varrho*(sigma_c-1)*(H_bar/(1-H_bar))*h;
    
    % Deliberate mistake in linearization - replace above with:
    //uc=((1-varrho)*(1-sigma_c)-1)*((c-chii*c(-1))/(1-chii))+ (sigma_c-1)*(H_bar/(1-H_bar))*h; 
